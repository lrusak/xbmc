  ...
  if (!m_context)
  {
    ...
    if (result == 0 || (errno == EEXIST && IsFirstInit))
    {
      ...
      if (f != NULL)
      {
        ...
        if (settings->GetInt(SMB::SETTING_SMB_MINPROTOCOL) > 0)
        {
          if (settings->GetInt(SMB::SETTING_SMB_MINPROTOCOL) == 1)
            fprintf(f, "\tclient min protocol = NT1\n");
          else
            fprintf(f, "\tclient min protocol = SMB%d\n",
                    settings->GetInt(SMB::SETTING_SMB_MINPROTOCOL));
        }

        // set maximum smbclient protocol version
        if (settings->GetInt(SMB::SETTING_SMB_MAXPROTOCOL) > 0)
        {
          if (settings->GetInt(SMB::SETTING_SMB_MAXPROTOCOL) == 1)
            fprintf(f, "\tclient max protocol = NT1\n");
          else
            fprintf(f, "\tclient max protocol = SMB%d\n",
                    settings->GetInt(SMB::SETTING_SMB_MAXPROTOCOL));
        }

        // set legacy security options
        if (settings->GetBool(SMB::SETTING_SMB_LEGACYSECURITY) &&
            (settings->GetInt(SMB::SETTING_SMB_MAXPROTOCOL) == 1))
        {
        ...
        if (settings->GetString(SMB::SETTING_SMB_WINSSERVER).length() > 0 &&
            !StringUtils::EqualsNoCase(settings->GetString(SMB::SETTING_SMB_WINSSERVER), "0.0.0.0"))
        {
          fprintf(f, "\twins server = %s\n",
                  settings->GetString(SMB::SETTING_SMB_WINSSERVER).c_str());
    ...
    if (settings->GetString(SMB::SETTING_SMB_WORKGROUP).length() > 0)
      //! @bug libsmbclient < 4.9 isn't const correct
      smbc_setWorkgroup(m_context,
                        const_cast<char*>(settings->GetString(SMB::SETTING_SMB_WORKGROUP).c_str()));
    ...
    if (settings->GetString(SMB::SETTING_SMB_WORKGROUP).length() > 0)
      m_context->workgroup = strdup(settings->GetString(SMB::SETTING_SMB_WORKGROUP).c_str());
