...
int GetNativeThreadPriority(const ThreadPriority priority)
{
...
int GetUserMaxPriority(int maxPriority)
{
...
{
  return std::make_unique<CThreadImplLinux>(handle, name);
}

CThreadImplLinux::CThreadImplLinux(std::thread::native_handle_type handle, const std::string& name)
  : IThreadImpl(handle, name)
{
  pthread_setname_np(m_handle, m_name.c_str());

  // get user max prio
  int userMaxPrio = GetUserMaxPriority(GetNativeThreadPriority(ThreadPriority::HIGHEST));
  ...
  if (userMaxPrio > 0)
  {
    ...
    if (setpriority(PRIO_PROCESS, m_handle, appNice) != 0)
...
bool CThreadImplLinux::SetPriority(const ThreadPriority priority)
{
  ...
  int prio = GetNativeThreadPriority(priority);
  if (prio >= GetNativeThreadPriority(ThreadPriority::HIGHEST))
    prio = userMaxPrio; // this is already the min of GetMaxPriority and what the user can set.
  if (prio < GetNativeThreadPriority(ThreadPriority::LOWEST))
    prio = GetNativeThreadPriority(ThreadPriority::LOWEST);
  ...
  if (setpriority(PRIO_PROCESS, m_handle, newNice) != 0)
  {
    CLog::Log(LOGERROR, "{}: error {}", __FUNCTION__, strerror(errno));
    return false;
  }
