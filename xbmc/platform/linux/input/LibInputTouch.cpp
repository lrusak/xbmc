...
{
  const double x = libinput_event_touch_get_x_transformed(e, g_graphicsContext.GetWidth());
  const double y = libinput_event_touch_get_y_transformed(e, g_graphicsContext.GetHeight());
...
void CLibInputTouch::ProcessTouchFrame(libinput_event_touch *e)
{
  ...
  for (size_t slot = 0; slot < m_points.size(); ++slot)
  {
    ...
    CLog::Log(LOGDEBUG, "CLibInputTouch::%s - touch input frame: x %f", __FUNCTION__, m_points.at(slot).second.x);
    CLog::Log(LOGDEBUG, "CLibInputTouch::%s - touch input frame: y %f", __FUNCTION__, m_points.at(slot).second.y);
