...
bool CMusicDatabase::AddAlbum(CAlbum& album, int idSource)
{
  ...
  for (auto song = album.songs.begin(); song != album.songs.end(); ++song)
  {
    ...
    song->idSong = AddSong(
        song->idSong, song->dateNew, song->idAlbum, song->strTitle, song->strMusicBrainzTrackID,
        song->strFileName, song->strComment, song->strMood, song->strThumb, song->GetArtistString(),
        song->GetArtistSort(), song->genre, song->iTrack, song->iDuration, song->strReleaseDate,
        song->strOrigReleaseDate, song->strDiscSubtitle, song->iTimesPlayed, song->iStartOffset,
        song->iEndOffset, song->lastPlayed, song->rating, song->userrating, song->votes, song->iBPM,
        song->iBitRate, song->iSampleRate, song->iChannels, song->replayGain);
...
bool CMusicDatabase::UpdateAlbum(CAlbum& album)
{
  ...
  UpdateAlbum(
      album.idAlbum, album.strAlbum, album.strMusicBrainzAlbumID, album.strReleaseGroupMBID,
      album.GetAlbumArtistString(), album.GetAlbumArtistSort(), album.GetGenreString(),
      StringUtils::Join(album.moods, itemSeparator), StringUtils::Join(album.styles, itemSeparator),
      StringUtils::Join(album.themes, itemSeparator), album.strReview, album.thumbURL.GetData(),
      album.strLabel, album.strType, album.strReleaseStatus, album.fRating, album.iUserrating,
      album.iVotes, album.strReleaseDate, album.strOrigReleaseDate, album.bBoxedSet,
      album.bCompilation, album.releaseType, album.bScrapedMBID);
  ...
  try
  {
    ...
    else
    {
      ...
      UpdateSong(idNew, strTitle, strMusicBrainzTrackID, strPathAndFileName, strComment, strMood,
                 strThumb, artistDisp, artistSort, genres, iTrack, iDuration, strReleaseDate,
                 strOrigReleaseDate, strDiscSubtitle, iTimesPlayed, iStartOffset, iEndOffset,
                 dtLastPlayed, rating, userrating, votes, replayGain, iBPM, iBitRate, iSampleRate,
                 iChannels);
    }
    if (!strThumb.empty())
  ...
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      strSQL = PrepareSQL(
          ...
          strAlbum.c_str(), strArtist.c_str(), strGenre.c_str(), strReleaseDate.c_str(),
          strOrigReleaseDate.c_str(), bBoxedSet, strRecordLabel.c_str(), strType.c_str(),
          strReleaseStatus.c_str(), bCompilation, CAlbum::ReleaseTypeToString(releaseType).c_str());
