...
{
  KODI::WINDOWING::CWindowSystemFactory::RegisterWindowSystem(CreateWinSystem, "x11", "gl");
}
...
void* CWinSystemX11GLContext::GetProcAddress(const char* name)
{
  if (!m_pGLContext)
    return nullptr;

  return m_pGLContext->GetProcAddress(name);
}
