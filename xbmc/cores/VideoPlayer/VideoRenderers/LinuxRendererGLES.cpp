...
void CLinuxRendererGLES::UpdateVideoFilter()
{
  ...
  case VS_SCALINGMETHOD_CUBIC_0_1:
  {
    ...
    auto shader = static_cast<Shaders::GLES::ConvolutionFilterShader*>(
        shaderCache->Find(m_pVideoFilterShader));
    if (shader)
    {
    ...
    switch(requestedMethod)
    {
      ...
      case RENDER_METHOD_GLSL:
      {
        ...
        if (glCreateProgram())
        {
          ...
          auto shader = static_cast<Shaders::GLES::YUV2RGBProgressiveShader*>(
              shaderCache->Find(m_pYUVProgShader));
          if (shader)
          {
