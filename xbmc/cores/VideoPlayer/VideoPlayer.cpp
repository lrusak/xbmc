  ...
  while (!m_bAbortRequest)
  {
    ...
    if (!pPacket)
    {
      ...
      if (std::shared_ptr<CDVDInputStream::IMenus> pStream = std::dynamic_pointer_cast<CDVDInputStream::IMenus>(m_pInputStream))
      {
        ...
        {
          if (m_dvd.iDVDStillTime.count() > 0)
          {
            if ((std::chrono::steady_clock::now() - m_dvd.iDVDStillStartTime) >=
                m_dvd.iDVDStillTime)
            {
              m_dvd.iDVDStillTime = {};
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY))
  {
    ...
    case BD_EVENT_STILL_TIME:
    {
      if (m_dvd.state != DVDSTATE_STILL)
      {
        ...
        if (m_dvd.iDVDStillTime.count() > 0)
          m_dvd.iDVDStillTime = std::chrono::milliseconds(m_dvd.iDVDStillTime.count() * 1000);

        /* adjust for the output delay in the video queue */
        unsigned int time = 0;
        if (m_CurrentVideo.stream && m_dvd.iDVDStillTime.count() > 0)
        {
          time = (unsigned int)(m_VideoPlayerVideo->GetOutputDelay() / (DVD_TIME_BASE / 1000));
          if (time < 10000 && time > 0)
            m_dvd.iDVDStillTime += std::chrono::milliseconds(time);
        }
        m_dvd.state = DVDSTATE_STILL;
        CLog::Log(LOGDEBUG, "BD_EVENT_STILL_TIME - waiting %i msec, with delay of %d sec",
                  m_dvd.iDVDStillTime.count(), time / 1000);
      }
    }
    break;
    case BD_EVENT_STILL:
    ...
    case DVDNAV_STILL_FRAME:
      {
        ...
        if (m_dvd.state != DVDSTATE_STILL)
        {
          ...
          if(still_event->length < 0xff)
            m_dvd.iDVDStillTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::seconds(still_event->length));
          else
            m_dvd.iDVDStillTime = {};
...
#define THREAD_ACTION(action) \
  ...
  if (pMenus)
  {
    if (m_dvd.state == DVDSTATE_STILL && m_dvd.iDVDStillTime.count() != 0 &&
        ...
        case ACTION_SELECT_ITEM:
          {
            ...
            m_dvd.iDVDStillStartTime = {};
            m_dvd.iDVDStillTime = std::chrono::milliseconds(1);
          }
          return true;
      }
    }
...
void CVideoPlayer::UpdatePlayState(double timeout)
{
  ...
  if (m_pInputStream)
  {
    ...
    if (pMenu)
    {
      ...
      if (m_dvd.state == DVDSTATE_STILL)
      {
        state.time = std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now() - m_dvd.iDVDStillStartTime)
                         .count();
