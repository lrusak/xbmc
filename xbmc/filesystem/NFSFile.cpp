...
struct nfs_context *CNfsConnection::getContextFromMap(const std::string &exportname, bool forceCacheHit/* = false*/)
{
  ...
  if (it != m_openContextMap.end())
  {
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - it->second.lastAccessedTime)
                .count() < CONTEXT_TIMEOUT ||
        forceCacheHit)
    {
      ...
      if (!forceCacheHit) // only log it if this isn't the resetkeepalive on each read ;)
        CLog::Log(LOGDEBUG, "NFS: Refreshing context for {}, old: %" PRId64 ", new: %" PRId64,
                  exportname.c_str(), it->second.lastAccessedTime.time_since_epoch().count(),
